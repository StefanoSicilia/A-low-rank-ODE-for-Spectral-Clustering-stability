function [U,S,info]=OuterIter_LR(W,k,method_ii,method_oi)
%% OuterIter_LR: Spectral Graph Clustering Robustness (Low Rank version)
...
% argmin_{epsilon>0} F_epsilon(E_star(epsilon)),
% where E_star is the solution of the eigenvalue optimization problem
% argmin_{norm(E,'fro')=1, E=E' and has same pattern of W} F_epsilon(E).
...
% InnerIter_LR (see this function for more details), while the outer 
% iteration uses a Newton-Bisection method to determine the size of the 
% perturbation epsilon_star.
%
% INPUTS
...
% InnerIter_FR and all its subfunctions.
    ...
    %% OUTER ITERATIONS
    [U,S,info_ii]=InnerIter_LR(W,epsilon,k,method_ii,U,S);
    g=info_ii.derfeps;
    ...
    disp(['Outer iterations: ',num2str(j),' out of ',num2str(niter),...
        ' for distance ',num2str(d(end)),'.'])
    pattern=struct('row',row,'col',col);
    info=struct('objfun',f,'outiter',j,'d',d,'neigs',neigsout,...
        'dlowerboundeps',el,'dupperbound',eu,'pattern',pattern);
